...
from .constants import (GLOBAL_HEADERS, X_REQUEST_ID, X_SOURCE_IP,
                        X_SOURCE_REFERER, X_SOURCE_USER_AGENT, X_USER_AGENT,
                        X_VISITOR_ID)
...
async def handle_request_id(request):
    context.set(X_REQUEST_ID, get_request_id_from_request(request))
...
async def add_response_time(request, response):
    context.set(
        "response_time", int(time.time() * 1000) - context.get("request_start_time")
    )
